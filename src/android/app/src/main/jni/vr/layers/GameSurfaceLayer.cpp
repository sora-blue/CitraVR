...
XrPosef CreateTopPanelFromWorld(const XrVector3f& position) {
    ...
    if (isRotatedAnticlockwise) {
        ...
        constexpr XrVector3f upPosition     = {-1, 0, 0};
        pose.orientation = CalculatePanelRotation(pose.position, playerPosition, upPosition);
...
// notified by menu/settings, switch between horizontal/vertical
void GameSurfaceLayer::SwitchPanelsOrientation() {
